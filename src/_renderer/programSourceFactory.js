(function () {
    ...
    xeogl.renderer.ProgramSourceFactory = new (function () {
            ...
            this.getSource = function (hash, _states) {
                ...
                pbrSpecularGlossinessMaterial = (states.material.type === "PBRSpecularGlossinessMaterial");
            ...
            function vertexDraw() {
                ...
                if (texturing) {
                    add("attribute vec2 xeo_aUV;"); // Vertex UV coordinate
                    add("varying vec2 xeo_vUV;"); // Fragment UV coordinate
                }

                if (states.geometry.colors) {
                    add("attribute vec4 xeo_aColor;"); // Vertex color
                    add("varying vec4 xeo_vColor;"); // Fragment color
                }
                ...
                if (normals) {
                    ...
                    if (normalMapping) { // Compute the tangent-bitangent-normal (TBN) matrix
                    ...
                    for (i = 0, len = states.lights.lights.length; i < len; i++) { // Lights
                        ...
                        if (light.type === "dir") { // Directional light
                            if (light.space === "world") { // World space light
                                add("tmpVec3 = xeo_uLightDir" + i + ";");
                                add("tmpVec3 = vec3(viewMatrix * vec4(tmpVec3, 1.0)).xyz;"); // Transform to View space
                                if (normalMapping) { // Transform to Tangent space
                                    add("   tmpVec3 *= TBN;");
                                }
                            } else { // View space light
                                add("tmpVec3 = xeo_uLightDir" + i + ";");
                                if (normalMapping) { // Transform to Tangent space
                                    add("tmpVec3 *= TBN;");
                                }
                        ...
                        if (light.type === "point") { // Positional light
                            if (light.space === "world") { // World space
                                ...
                                add("lightDist = abs(length(tmpVec3));"); // Get distance to light
                                if (normalMapping) { // Transform light vector to Tangent space
                                    add("tmpVec3 *= TBN;");
                                }
                            } else { // View space
                                // Get vertex -> light vector in View space
                                ...
                                add("lightDist = abs(length(tmpVec3));"); // Get distance to light
                                if (normalMapping) {
                                    add("tmpVec3 *= TBN;"); // Transform light vector to tangent space
                                }
                            }
                    ...
                    if (normalMapping) {
                        add("xeo_vViewEyeVec *= TBN;");// Transform vertex->eye vector to tangent space
                    }
                }
            ...
            function fragmentDraw() {
                ...
                if (normals) {
                    add("varying vec4 xeo_vViewPosition;");
                    add("varying vec3 xeo_vViewEyeVec;"); // View-space vector from fragment to eye
                    add("varying vec3 xeo_vViewNormal;"); // View-space fragment normal
                    for (i = 0, len = states.lights.lights.length; i < len; i++) { // Light sources
                        light = states.lights.lights[i];
                        if (light.type === "ambient") {
                            continue;
                        }
                ...
                if (pbrSpecularGlossinessMaterial) {
                    add("uniform vec4 xeo_uDiffuseFactor;");
                    if (normals) {
                        add("uniform vec3 xeo_uSpecularFactor;");
                        add("uniform float xeo_uGlossinessFactor;");
                    }
                    if (texturing) {
                        if (states.material.diffuseMap) {
                            add("uniform sampler2D xeo_uDiffuseMap;");
                        ...
                        if (normals) {
                            if (states.material.specularGlossinessMap) {
                                add("uniform sampler2D xeo_uSpecularGlossinessMap;");
                                if (states.material.specularGlossinessMap.matrix) {
                ...
                if (phongMaterial) {
                    ...
                    if (normals) {
                        ...
                        if (normalMapping) {
                            add("vec3 viewNormal = vec3(0.0, 1.0, 1.0);");
                        } else {
                    ...
                    if (texturing) {
                        ...
                        // Opacity and emissive and diffuse lighting and mapping are independent of normals
                    ...
                    if (normals) {
                        ...
                        for (i = 0, len = states.lights.lights.length; i < len; i++) {
                            ...
                            add("viewLightVec = normalize(xeo_vViewLightVecAndDist" + i + ".xyz);"); // If normal mapping, the fragment->light vector will be in tangent space
                            if (light.type === "point") {
                                add("specAngle = max(dot(viewNormal, viewLightVec), 0.0);");
                                add("lightDist = xeo_vViewLightVecAndDist" + i + ".w;");
                                add("attenuation = 1.0 - (xeo_uLightAttenuation" + i + "[0] + xeo_uLightAttenuation" + i + "[1] * lightDist + xeo_uLightAttenuation" + i + "[2] * lightDist * lightDist);");
                ...
                if (pbrSpecularGlossinessMaterial) {
                    ...
                    if (states.geometry.colors) {
                        ...
                        add("vec3 diffuse = xeo_uDiffuseFactor.rgb;");
                    }

                    if (normals) {
                        add("vec3 specularFactor = xeo_uSpecularFactor;");
                        add("float glossiness = xeo_uGlossinessFactor;");
                        if (normalMapping) {
                    ...
                    if (texturing) {
                        ...
                        if (normals) {
                            if (material.specularGlossinessMap) {
                                ...
                                add("textureCoord.y = -textureCoord.y;");
                                add("glossiness = texture2D(xeo_uSpecularGlossinessMap, textureCoord).rgb;");
                            }
                        }
                    ...
                    if (normals) {
                        ...
                        add("float lightDist;");
                        add("float attenuation;");
                        add("float shininess = xeo_uGlossinessFactor;");
                        add("vec3 specular = xeo_uSpecularFactor;");
                        ...
                        add("fragColor = vec4((specular * specularLight) + ((diffuseLight + (ambient * xeo_uLightAmbientIntensity) ) * diffuse), opacity);");
                    } else {
                        add("fragColor = vec4(emissive + diffuse, opacity);");
                    }
                } // PBRSpecularGlossinessMaterial
