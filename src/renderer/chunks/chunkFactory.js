 ...
 */
XEO.ChunkFactory = function() {
    this._chunks = {};
    this.chunkTypes = XEO.ChunkFactory.chunkTypes;
};

/**
 * Sub-classes of {@link XEO.Chunk} provided by this factory
 */
XEO.ChunkFactory.chunkTypes = {};    // Supported chunk classes, installed by #createChunkType

/**
 * Free pool of unused {@link XEO.Chunk} instances
 */
XEO.ChunkFactory._freeChunks = {};    // Free chunk pool for each type
 ...
 */
XEO.ChunkFactory.createChunkType = function(params) {
    ...
    XEO.ChunkFactory.chunkTypes[params.type] = chunkClass;

    XEO._apply(params, chunkClass.prototype);   // Augment subclass

    XEO.ChunkFactory._freeChunks[params.type] = { // Set up free chunk pool for this type
        chunks: [],
        chunksLen: 0
    };
 ...
 *
 */
XEO.ChunkFactory.prototype.getChunk = function(chunkId, type, program, core, core2) {

    var chunkClass = XEO.ChunkFactory.chunkTypes[type]; // Check type supported
    ...
    var freeChunks = XEO.ChunkFactory._freeChunks[type]; // Try to recycle a free chunk
 ...
 */
XEO.ChunkFactory.prototype.putChunk = function (chunk) {
    ...
    if (--chunk.useCount <= 0) {    // Release shared core if use count now zero
        ...
        var freeChunks = XEO.ChunkFactory._freeChunks[chunk.type];
 ...
 */
XEO.ChunkFactory.prototype.webglRestored = function () {
